...
{
  // console.log('sortable-1.13.0 is loaded');
  var el         = $('[data-sortable]');
  var mySortable = null;
  // logy(el);
  ...
  if(el.length === 1)
  {
    ...
    mySortable = Sortable.create(el,
    {
      ...
      onUpdate: function (e)
      {
        ...
        else if($(el).parents('form').length === 1)
        {
          $(el).parents('form').ajaxify();
        }
      }
    });
  }
  ...
  {
    // console.log('sortable1 nested')
    for (var i = 0; i < el.length; i++)
    {
      var mySortableElement = el[i];

      new Sortable(mySortableElement,
      {
        handle: '.sortHandle',
        animation: 150,
        filter: ".fix",
        ...
        onUpdate: function (e)
        {
          ...
          // someChangeOnSortable(mySortableElement);
        },
        onEnd:function (e)
        {
          console.log('end');
...
function someChangeOnSortable(_el)
{
  updateNestedMenuSort();
  ...
  else if($(_el).parents('form').length === 1)
  {
    myParentForm = $(_el).parents('form');
  }

  if(myParentForm.attr('data-patch') !== undefined)
  {
    myParentForm.ajaxify();
  }
}


function updateNestedMenuSort()
{
  mySortable = $('.items2[data-sortable]');

  mySortable.find('> li').each(function(_index1)
  {
    var l1 = $(this);
    var l1_item = l1.find('> .item input');
    var l1_val = _index1 + '_' + l1_item.attr('data-id');
    l1_item.attr('value', l1_val);

    l1.find('> ol > li').each(function(_index2)
    {
      var l2 = $(this);
      var l2_item = l2.find('> .item input');
      var l2_val = l1_val + '-' + _index2 + '_' + l2_item.attr('data-id');
      l2_item.attr('value', l2_val);

      l2.find('> ol > li').each(function(_index3)
      {
        var l3 = $(this);
        var l3_item = l3.find('> .item input');
        var l3_val = l2_val + '-' + _index3 + '_' + l3_item.attr('data-id');
        l3_item.attr('value', l3_val);

        l3.find('> ol > li').each(function(_index4)
        {
          var l4 = $(this);
          var l4_item = l4.find('> .item input');
          var l4_val = l3_val + '-' + _index4 + '_' + l4_item.attr('data-id');
          l4_item.attr('value', l4_val);

          l4.find('> ol > li').each(function(_index5)
          {
            var l5 = $(this);
            var l5_item = l5.find('> .item input');
            var l5_val = l4_val + '-' + _index5 + '_' + l5_item.attr('data-id');
            l5_item.attr('value', l5_val);
          });
        });
      });
    });
