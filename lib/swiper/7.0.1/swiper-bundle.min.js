...
function runPageSwiperSlider()
{
  ...
  // use auto size for each slide
  opt.slidesPerView = 'auto';

  $('.swiper').fadeIn();

  // run slider
  if($('.swiper[data-effect="fade"]').length > 0)
  {
    // add extra settings
    optFade = opt;
    optFade.effect = 'fade';
    ...
    const swiperFade = new Swiper('.swiper[data-effect="fade"]', optFade);
  }

  if($('.swiper[data-effect="coverflow"]').length > 0)
  {
    // add extra settings
    optCoverflow = opt;
    optCoverflow.effect = 'coverflow';
    optCoverflow.coverflowEffect = {
      rotate: 30,
    };
    const swiperCoverflow = new Swiper('.swiper[data-effect="coverflow"]', optCoverflow);
  }

  if($('.swiper[data-effect="flip"]').length > 0)
  {
    // add extra settings
    optFlip = opt;
    optFlip.effect = 'flip';
    ...
    const swiperFlip = new Swiper('.swiper[data-effect="flip"]', optFlip);
  }

  if($('.swiper[data-effect="cube"]').length > 0)
  {
    // add extra settings
    optCube = opt;
    optCube.effect = 'cube';
    ...
    const swiperCube = new Swiper('.swiper[data-effect="cube"]', optCube);
  }


  if($('.swiper[data-effect="cards"]').length > 0)
  {
    // add extra settings
    optCards = opt;
    optCards.effect = 'cards';
    // optCards.cardsEffect = {
    //   slideShadows: false,
    // };
    const swiperCards = new Swiper('.swiper[data-effect="cards"]', optCards);
  }

  if($('.swiper[data-effect="creative"]').length > 0)
  {
    // add extra settings
    optCreative = opt;
    optCreative.effect = 'custom';
    optCreative.creativeEffect = {
      prev: {
        shadow: true,
        translate: ["-120%", 0, -500],
      },
    ...
    const swiperCreative = new Swiper('.swiper[data-effect="creative"]', optCreative);
  }

  if($('.swiper[data-effect="slide"]').length > 0)
  {
    const swiperSlide = new Swiper('.swiper[data-effect="slide"]', opt);
  }
}
