...
function runSortable2()
{
  console.log('sortable2 is loaded');
  var elSortContainer = $('[data-sortable2]');
  var mySortable = null;
  // console.log(elSortContainer);

  if(elSortContainer.length > 0)
  {
    var oldContainer;
    elSortContainer.sortable(
    {
      group: 'nested',
      nested: true,
      handle: '[data-handle]',
      onDrop: function ($item, _container, _super)
      {
        _container.el.removeClass("active");
        _super($item, _container);

        myParentForm = $(elSortContainer).parents('form');
        if(elSortContainer.tagName === "FORM")
        {
          $(elSortContainer).ajaxify();
        }
        else if(myParentForm.length === 1)
        {
